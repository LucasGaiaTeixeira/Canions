...
public class CsTerrainController : EditorWindow {
	...
	private void GenerateAndExportTerrainMesh()
	{
		...
		if (!string.IsNullOrEmpty(path))
		{
			...
			// Export the GameObject as an FBX file via reflection helper
			if (TerrainFbxExporter.TryExportGameObjectToFbx(path, meshObj))
			{
				Debug.Log("Mesh exported to: " + path);
			}
			else
			{
				EditorUtility.DisplayDialog("FBX Export Error", "FBX export failed. Install the FBX Exporter package (com.unity.formats.fbx) or check the Console.", "OK");
			}
